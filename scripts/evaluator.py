"""Inversion quality metrics and summary figures from ``*_inverse_init.npz``."""
import sys
...
parser.add_argument("data_path", type=str, nargs='?', default=_DEFAULT_NPZ)
...
# Report filenames derived from NPZ path
import os
...
from spinflow.fd_model import PHASES, PHASE_DISPLAY, PHASE_COLORS
...
phase_names = [PHASE_DISPLAY[p] for p in PHASES]
...
# === Row 1: Flow Analysis ===
# Panel: observed vs predicted flow
...
# Panel: FD cloud with prototype curves
...
# Panel: observed vs predicted speed
...
for i, phase in enumerate(PHASES):
    ax7.plot(
        x_pos,
        pi_final[:, i],
        label=f"π_{PHASE_DISPLAY[phase]} (vf={prototypes[i].vf*3.6:.1f})",
        linewidth=2.5,
        alpha=0.8,
